...
const Dashboard: React.FC = () => {
  ...
  const normalizeData = (data: ExcelRow[]): ExcelRow[] => {
    ...
    return data.map(row => {
      ...
      // Second pass: Merge Description with Code
      if (normalizedRow['Description'] && normalizedRow['Code']) {
        const desc = String(normalizedRow['Description'] || '').trim();
        const code = String(normalizedRow['Code'] || '').trim();
        if (desc && code) {
          normalizedRow['Description'] = `${desc} ${code}`;
        }
        // Remove the Code column since it's merged
        delete normalizedRow['Code'];
      }
      
      // Third pass: Merge Quantity with Unit (if Unit column exists)
      if (normalizedRow['Quantity']) {
        const qty = String(normalizedRow['Quantity'] || '').trim();
        const unit = normalizedRow['Unit'] ? String(normalizedRow['Unit']).trim() : '';
        if (qty && unit) {
          normalizedRow['Quantity'] = `${qty} ${unit}`;
          // Remove the Unit column since it's merged
