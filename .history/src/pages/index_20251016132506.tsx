...
const Dashboard: React.FC = () => {
  ...
  const findHeaderRow = (data: ExcelRow[]): number => {
    // Find the row that contains "Seq" or "Seq." in the first few columns
    for (let i = 0; i < Math.min(data.length, 30); i++) {
      const row = data[i];
      const values = Object.values(row).map(v => String(v).toLowerCase());
      
      // Check if this row has "seq" and "description" - likely the header row
      const hasSeq = values.some(v => v.includes('seq'));
      const hasDesc = values.some(v => v.includes('desc'));
    ...
    return -1; // Not found, assume first row
  };
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // First read all data to find header row
          ...
          if (headerRowIndex === -1) {
            ...
            // Read starting from the header row
            const jsonData = XLSX.utils.sheet_to_json(worksheet, { 
  ...
  const normalizeColumnName = (name: string, index: number, allKeys: string[]): string => {
    // Normalize column names to handle variations
    const normalized = name.trim().toLowerCase().replace(/\s+/g, ' ');
    
    // Handle __EMPTY columns by position (Excel with no headers)
    if (normalized.includes('__empty') || normalized === '') {
      // Count how many __EMPTY columns come before this one
      const emptyIndex = allKeys.slice(0, index).filter(k => 
        k.trim().toLowerCase().includes('__empty') || k.trim() === ''
      ).length;
      
      // Map by position: 0=Seq, 1=Description, 2=Part Number, 3=Quantity, 4=Unit, etc.
      if (emptyIndex === 0) return 'Seq.';
      if (emptyIndex === 1) return 'Description';
      if (emptyIndex === 2) return 'Quantity';
      if (emptyIndex === 3) return 'Unit';
      if (emptyIndex === 4) return 'Unit Price';
      if (emptyIndex === 5) return 'Amount';
      return `Column${emptyIndex}`;
    }
  ...
  const normalizeData = (data: ExcelRow[]): ExcelRow[] => {
    ...
    return data.map(row => {
      ...
      allKeys.forEach((key, index) => {
        const normalizedKey = normalizeColumnName(key, index, allKeys);
