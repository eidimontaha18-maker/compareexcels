...
const Dashboard: React.FC = () => {
        ...
        try {
          ...
          if (headerRowIndex === -1) {
            ...
            const columnNames = headerRow.map((header, index) => {
              ...
              if (headerStr === '') {
                // Empty header - this is the Code column (part numbers)
                // It's typically the 3rd column (index 2) between Description and Quantity
                // But we check by looking at adjacent columns to be sure
                const prevHeader = index > 0 ? String(headerRow[index - 1] || '').toLowerCase() : '';
                const nextHeader = index < headerRow.length - 1 ? String(headerRow[index + 1] || '').toLowerCase() : '';
                
                // If previous column is Description-like and next is Quantity-like, this is Code
                if ((prevHeader.includes('desc') || prevHeader.includes('description')) && 
                    (nextHeader.includes('qty') || nextHeader.includes('quantity'))) {
                  return 'Code';
                }
                // Default to Code for any empty column in this context
                return 'Code';
              }
              return headerStr;
            });
  ...
  const normalizeColumnName = (name: string): string => {
    ...
    // Handle empty columns or __EMPTY columns (Excel with no headers)
    if (normalized.includes('__empty') || normalized === '' || normalized === 'code') {
      // The column without a header is the CODE column (part numbers)
      // It appears between Description and Quantity in the Excel file
      return 'Code';
    }
    
    // Check for Seq variations (case-insensitive)
    if (normalized.includes('seq')) return 'Seq.';
    
    // Check for Description variations (case-insensitive)
    if (normalized.includes('description') || normalized.includes('desc')) return 'Description';
    
    // Check for Quantity variations (handles both "Qty" and "Quantity")
  ...
  const normalizeValue = (value: string | number | boolean | null | undefined): string => {
    ...
    if (numberMatch) {
      ...
      if (!isNaN(numValue)) {
        // Return just the number for comparison (normalize "8 PCS" and "8" to both be "8")
    ...
    <div className="dashboard">
      ...
      <div className="dashboard-content">
        <header className="page-header">
          ...
          <p>Matches items by <strong>Seq. + Description</strong>, then detects changes in <strong>Quantity (Qty/PCS), Unit Price, and Amount</strong></p>
        </header>
        ...
        {comparisonResult && (
          <div className="summary-card">
            ...
            <p>Files compared using <strong>Seq. + Description</strong> as matching keys. Changes detected in <strong>Quantity (Qty/PCS), Unit Price, and Amount</strong>.</p>
          ...
          <>
            <div className="filter-section">
              ...
              >
                ...
                <span className="filter-label">Cancel</span>
              ...
              >
                ...
                <span className="filter-label">Ok</span>
